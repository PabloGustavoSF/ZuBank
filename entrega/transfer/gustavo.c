...
void exibirTransferencia(Transferencia **vetorTransf, int count)
{
...
void deletarTransferencia(Transferencia *transf)
{
    if (transf != NULL)
    {
        free(transf);
        printf("Transfer�ncia exclu�da com sucesso.\n");
    }
}
...
void menuTransferencia(Transferencia **vetorTransf, Cliente *cliente, int max_transf)
{
    ...
    float valor;
    char remetente[50];
    FILE *arquivo = fopen("transferencias.txt", "a");
...
Investimento *criarInvestimento(float valor, int meses)
{
    ...
    {
        printf("Erro ao alocar mem�ria para o investimento.\n");
        return NULL;
    }
    investimento->investimento = valor;
    investimento->meses_investidos = meses;
    return investimento;
}
...
void exibirInvestimento(const Investimento *investimento)
{
    if (investimento == NULL)
        return;
    printf("Valor Investido: R$%.2f\nMeses: %d\nRendimento Bruto: R$%.2f\nRendimento L�quido: R$%.2f\n",
           investimento->investimento, investimento->meses_investidos,
           investimento->rendimento_bruto, investimento->rendimento_liquido);
}
...
void deletarInvestimento(Investimento *investimento)
{
    ...
    {
        free(investimento);
        printf("Investimento exclu�do com sucesso.\n");
    }
}
